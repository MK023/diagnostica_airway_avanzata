...
@pytest.mark.parametrize(
    "address,expected", [("8.8.8.8", True), ("invalid_address", False)]
)
def test_run_ping_diag_basic(monkeypatch, address, expected):
    # Mock ping3_ping
    monkeypatch.setattr(
        "network.ping.ping3_ping", lambda addr, unit: 42 if addr == "8.8.8.8" else None
    )
    ...
    class DummyStats:
        def as_dict(self):
            return {"min_rtt": 1, "avg_rtt": 2, "max_rtt": 3, "packet_loss_rate": 0}
    ...
    monkeypatch.setattr(
        "network.ping.pingparsing",
        type(
            "Dummy",
            (),
            {"PingParsing": DummyPingParsing, "PingTransmitter": DummyTransmitter},
        ),
    )
    # Mock scapy
    monkeypatch.setattr(
        "network.ping.sr1",
        lambda pkt, timeout, verbose: type("Dummy", (), {"time": 2, "sent_time": 1})(),
    )
    monkeypatch.setattr(
        "network.ping.IP", lambda dst: type("Dummy", (), {"sent_time": 1})()
    )
