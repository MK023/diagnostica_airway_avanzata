...
def test_run_dns_diag_valid(monkeypatch):
    ...
    monkeypatch.setattr("network.dns.socket.gethostbyaddr", lambda ip: ("testhost", [], [ip]))
    class DummyAnswer:
    ...
    class DummyResolver:
        ...
        def resolve(self, address, rtype):
            return [DummyAnswer()]
    monkeypatch.setattr("network.dns.dns", type("Dummy", (), {"resolver": type("Dummy2", (), {"Resolver": DummyResolver})}))
    run_dns_diag("google.com", DummyLogger())

def test_run_dns_diag_invalid(monkeypatch):
    monkeypatch.setattr("network.dns.socket.gethostbyname", lambda addr: (_ for _ in ()).throw(Exception("fail")))
